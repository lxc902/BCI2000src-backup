...
void WebcamLogger::Publish()
{
  ...
  BEGIN_PARAMETER_DEFINITIONS
    ...
    "Source:Webcam matrix WebcamFormat= 1 { camera%20index width height fps }"
      " 0 640 480 15 % %"
  ...
  for (int i = 0; i < NUM_OF_WEBCAM_EVENTS; i++) 
  {
    ...
    definition << "WebcamFrame" << i << " 24 0 0 0" << std::flush;
...
void WebcamLogger::Preflight() const
{
  ...
  if (Parameter("RecordAll")) {
    ...
    if (end > NUM_OF_WEBCAM_EVENTS)
      IndexTooLargeInfo();
    int n = CreateControllers(start, end, Parameter("WebcamFormat")).size();
    if (n == 0)
    ...
    if (idx > NUM_OF_WEBCAM_EVENTS)
      IndexTooLargeInfo();
    int n = CreateControllers(idx, idx, Parameter("WebcamFormat")).size();
    if (n == 0)
      bcierr << "No webcam with index " << idx << " has been found.";
  }
}
...
void WebcamLogger::Initialize()
{
  ...
  if (Parameter("RecordAll"))
    mControllers = CreateControllers(Parameter("StartIndex"), Parameter("EndIndex"), Parameter("WebcamFormat"));
  else
    mControllers = CreateControllers(Parameter("CameraNumber"), Parameter("CameraNumber"), Parameter("WebcamFormat"));
...
{
  std::string baseName = FileUtils::StripExtension(CurrentRun());
...
WebcamLogger::ControllerContainer WebcamLogger::CreateControllers(int startIndex, int endIndex, const ParamRef& formats)
{
  ...
  for (int camIndex = startIndex; camIndex <= endIndex; ++camIndex)
  {
    ...
    FindFormat(camIndex, formats, width, height, fps);
    pController->Initialize(camIndex, width, height, fps);
...
void WebcamLogger::FindFormat(int index, const ParamRef& formats, int& width, int& height, int& fps)
{
