...
void EyetrackerLoggerTobiiPro::SetGazeData(TobiiResearchGazeData const  &gaze)
{
	...
	WithLock(m_lock) 
	{
		...
		// Write states
		if(!sync_calculated && Statevector)
		{
...
TobiiResearchEyeTracker* EyetrackerLoggerTobiiPro::FindAndConnectToDevice()
{
	...
	{
		bcierr << "EyetrackerLoggerTobiiPro Error: Problem during device search: " << status;
		return NULL;
	}

	if(_trackers->count < 1)
	{
		bcierr << "EyetrackerLoggerTobiiPro Error: No device(s) found";
		return NULL;
	}
