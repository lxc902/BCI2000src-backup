////////////////////////////////////////////////////////////////////////////////
// $Id: GazeMonitorFilter.cpp 6543 2022-02-08 22:06:58Z mellinger $
  ...
  mStimuli ( false )
{
  ...
  BEGIN_PARAMETER_DEFINITIONS
    ...
    "Application:GazeMonitor list FixationX= {1} 0.5 0.5 0 1 "
      " // X coordinate of fixation center - 0.0 to 1.0 (expression)",
    "Application:GazeMonitor list FixationY= {1} 0.5 0.5 0 1 "
    ...
    "Application:GazeMonitor int ChangeFixation= 0 0 0 1 "
    " // Change fixation zone based on stimuli (boolean)",
...
GazeMonitorFilter::Preflight( const SignalProperties &Input, SignalProperties &Output ) const
{
  ...
  if( ( int )OptionalParameter( "LogEyetracker", 0 ) || ( int )OptionalParameter( "LogEyetrackerTobiiPro", 0 )
    || ( int )OptionalParameter( "LogEyetrackerTobii3", 0 ) || ( int )OptionalParameter("LogEyetrackerEyeLink", 0)
    || (int)OptionalParameter("LogEyetrackerTobiiX", 0) || (int)OptionalParameter("LogEyetrackerSimulator", 0))
  {
...
GazeMonitorFilter::Initialize( const SignalProperties &Input, const SignalProperties &Output )
{
  ...
  mStimuli = Parameter("ChangeFixation");
}
...
GazeMonitorFilter::Process( const GenericSignal &Input, GenericSignal &Output )
{
  ...
  if( mLoggingGaze )
  {
    ...
    gx *= mWidth;   gx /= ( 2.0f * 65535.0f );  gx -= mOffset; gx /= mScale;
    gy *= mHeight;  gy /= ( 2.0f * 65535.0f );  gy -= mOffset; gy /= mScale;
  }
  ...
  if( mEnforceFixation && mpAppDisplay )
  {
    ...
    if (mStimuli)
    {
      int stimNum = State("StimulusCode") - 1; //could be bad if StimulusCode==0?
      mFixationX = (Expression)Parameter("FixationX")(stimNum, 0);
      mFixationY = (Expression)Parameter("FixationY")(stimNum, 0);
    }
