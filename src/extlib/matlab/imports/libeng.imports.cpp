...
namespace {
// NOTE: Matlab must be in your system's PATH variable for the libraries to be found.
std::string FindMatlabLibdir()
{
  ...
  if(sizeof(void*) == 8)
    ...
    arch = "glnx86/";
#endif
  auto dirs = StringUtils::Split<std::vector<std::string>>(EnvVariable("PATH").Get(), ":");
  for(const auto& dir : dirs)
  {
    if(FileUtils::IsFile(dir + "/matlab") && FileUtils::IsDirectory(dir + "/" + arch))
    {
      path = dir + "/" + arch;
      break;
    }
