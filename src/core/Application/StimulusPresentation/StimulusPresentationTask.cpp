////////////////////////////////////////////////////////////////////////////////
// $Id: StimulusPresentationTask.cpp 6644 2022-03-29 17:05:55Z mellinger $
        ...
        for (int j = 0; j < Parameter(stimParams[i])->NumColumns(); ++j)
        {
            ...
            { // Check individual stimulus durations.
                ...
                if (value > 0 && (value < 1.0 || fmod(value, 1.0) > oneMillisecond))
                    bciout << "Due to a sample block duration of " << 1.0f / oneMillisecond << "ms,"
            ...
            {
                std::string file = Parameter(stimParams[i])(icon, j);
                if (!file.empty())
                    pImageStimulus->SetFile(file);
            }
            if (pAVStimulus != NULL)
            {
                std::string file = Parameter(stimParams[i])(av, j);
                if (!file.empty())
                {
