//////////////////////////////////////////////////////////////////////
// $Id: ParamDisplay.cpp 6708 2022-05-02 16:19:26Z mellinger $
...
////////////////////////////////////////////////////////////////////////////////
ParamDisplay::ParamDisplay() : mpDisplay(NULL)
{
}

ParamDisplay::ParamDisplay(const Param &inParam, QWidget *inParent) : mpDisplay(NULL)
{
...
ParamDisplay::ParamDisplay(const ParamDisplay &inOriginal) : mpDisplay(inOriginal.mpDisplay)
{
    if (mpDisplay)
        ++mpDisplay->mRefCount;
}

const ParamDisplay &ParamDisplay::operator=(const ParamDisplay &inOriginal)
{
...
void ParamDisplay::SetTop(int top)
{
    if (mpDisplay)
        mpDisplay->SetTop(top);
}

void ParamDisplay::SetLeft(int left)
{
    if (mpDisplay)
        mpDisplay->SetLeft(left);
}

int ParamDisplay::GetBottom()
{
    return mpDisplay ? mpDisplay->GetBottom() : 0;
}

int ParamDisplay::GetRight()
{
    return mpDisplay ? mpDisplay->GetRight() : 0;
}

void ParamDisplay::Hide()
{
    if (mpDisplay)
        mpDisplay->Hide();
}

void ParamDisplay::Show()
{
    if (mpDisplay)
        mpDisplay->Show();
}
...
ParamLabel::ParamLabel(QWidget *inParent, const ParsedComment &inParsedComment)
    : QLabel(inParent), mParamName(inParsedComment.Name()), mHelpContext(inParsedComment.HelpContext())
{
    setWhatsThis("dummy");
}
...
DisplayBase::DisplayBase(const ParsedComment &inParam, QWidget *inParent)
    : mRefCount(0), mModified(false), mTop(0), mLeft(0), mpUserLevel(NULL)
{
    if (!*Measures)
        InitMeasures();
...
void DisplayBase::SetTop(int inTop)
{
...
void DisplayBase::SetLeft(int inLeft)
{
...
int DisplayBase::GetBottom()
{
    int bottom = 0;
    for (WidgetIterator i = mWidgets.begin(); i != mWidgets.end(); ++i)
        bottom = std::max(bottom, (*i)->y() + (*i)->size().height());
    return bottom;
}

int DisplayBase::GetRight()
{
    int right = 0;
    for (WidgetIterator i = mWidgets.begin(); i != mWidgets.end(); ++i)
        right = std::max(right, (*i)->x() + (*i)->size().width());
    return right;
}

void DisplayBase::Hide()
{
    for (WidgetIterator i = mWidgets.begin(); i != mWidgets.end(); ++i)
        (*i)->hide();
}
...
bool DisplayBase::InitMeasures(float inDpi)
{
...
////////////////////////////////////////////////////////////////////////////////
SeparateComment::SeparateComment(const ParsedComment &inParam, QWidget *inParent) : DisplayBase(inParam, inParent)
{
    ...
    while (pComment->text() != ellipsis && pComment->x() + pComment->minimumSizeHint().width() >
                                               Measures[userlevelOffsetX_] + Measures[userlevelWidth_])
    {
...
SingleEntryEdit::SingleEntryEdit(const ParsedComment &inParam, QWidget *inParent)
    : SeparateComment(inParam, inParent), mpEdit(NULL)
{
...
Matrix::Matrix(const ParsedComment &inParam, QWidget *inParent)
    : SeparateComment(inParam, inParent), mKind(inParam.Kind()), mMatrixWindowOpen(false)
{
    ...
    pLoadButton->move(Measures[valueOffsetX_] + Measures[buttonWidth_] + Measures[buttonSpacingX_],
                      Measures[valueOffsetY_]);
    ...
    pSaveButton->move(Measures[valueOffsetX_] + 2 * (Measures[buttonWidth_] + Measures[buttonSpacingX_]),
                      Measures[valueOffsetY_]);
...
SingleEntryBlob::SingleEntryBlob(const ParsedComment &inParam, QWidget *inParent)
    : SeparateComment(inParam, inParent), mpData(0), mpParam(0), mKind(inParam.Kind())
{
    ...
    p = new QPushButton(inParent);
    p->setText("Save...");
    p->move(Measures[valueOffsetX_] + 2 * (Measures[buttonWidth_] + Measures[buttonSpacingX_]),
            Measures[valueOffsetY_]);
    p->resize(Measures[buttonWidth_], Measures[buttonHeight_]);
    ...
    : SingleEntryEdit(inParam, inParent)
{
    ...
    pButton->move(Measures[valueOffsetX_] + Measures[valueWidth_] + Measures[buttonHeight_] / 2,
                  Measures[valueOffsetY_]);
...
void SingleEntryInputFile::ButtonClick()
{
    QString fileName = QFileDialog::getOpenFileName(
        mpEdit->parentWidget(), tr("Choosing %1").arg(QString::fromLocal8Bit(mComment.c_str())),
        OperatorUtils::AppRelativeToAbsolutePath(mpEdit->text()).filePath(), ALLFILES_FILTER);
    if (!fileName.isEmpty())
...
////////////////////////////////////////////////////////////////////////////////
SingleEntryOutputFile::SingleEntryOutputFile(const ParsedComment &inParam, QWidget *inParent)
    : SingleEntryButton(inParam, inParent)
{
}
...
////////////////////////////////////////////////////////////////////////////////
SingleEntryDirectory::SingleEntryDirectory(const ParsedComment &inParam, QWidget *inParent)
    : SingleEntryButton(inParam, inParent)
{
}
...
////////////////////////////////////////////////////////////////////////////////
SingleEntryColor::SingleEntryColor(const ParsedComment &inParam, QWidget *inParent)
    : SingleEntryButton(inParam, inParent)
{
}
...
////////////////////////////////////////////////////////////////////////////////
SingleEntryEnum::SingleEntryEnum(const ParsedComment &inParam, QWidget *inParent)
    : SeparateComment(inParam, inParent), mpComboBox(NULL), mIndexBase(inParam.IndexBase())
{
...
////////////////////////////////////////////////////////////////////////////////
SingleEntryBoolean::SingleEntryBoolean(const ParsedComment &inParam, QWidget *inParent)
    : DisplayBase(inParam, inParent), mpCheckBox(NULL)
{
