////////////////////////////////////////////////////////////////////////////////
// $Id: VisDisplayGraph.cpp 6655 2022-04-01 17:50:10Z mellinger $
...
static const std::string cAutoScaleTime = "100ms";;

VisDisplayGraph::VisDisplayGraph(const std::string &inSourceID)
    : VisDisplayLayer(inSourceID), mpContextMenu(NULL), mpHPMenu(NULL), mpLPMenu(NULL), mpNotchMenu(NULL),
      mpActEnlargeSignal(NULL), mpActReduceSignal(NULL), mpActToggleAutoScale(NULL), mpActFewerSamples(NULL),
      mpActMoreSamples(NULL), mpActMoreChannels(NULL), mpActFewerChannels(NULL), mpActToggleDisplayMode(NULL),
      mpActToggleNumericValues(NULL), mpActToggleBaselines(NULL), mpActToggleValueUnit(NULL),
      mpActToggleChannelLabels(NULL), mpActToggleColor(NULL), mpActInvertDisplay(NULL), mpActChooseColors(NULL),
      mpActSmallerFont(nullptr), mpActLargerFont(nullptr), mNumChannels(0), mSignalElements(0), mUserScaling(0),
      mUserZoom(0), mpCurrentHPItem(NULL), mpCurrentLPItem(NULL), mpCurrentNotchItem(NULL), mpStatusBar(NULL),
      mpStatusLabel(NULL), mAutoScale(false), mAutoScaleTime(cAutoScaleTime), mFixedScale(false), mMinValue(-1), mMaxValue(1), mElementGain(0)
{
    BuildStatusBar();
    UpdateStatusBar();
    BuildContextMenu();
...
void VisDisplayGraph::OnSetConfig(ConfigSettings &inConfig)
{
    ...
    {
        double oldSampleUnit = NominalUnitsPerSample() * FilterUnitToValue(mDisplay.SampleUnit());
        PhysicalUnit u;
    ...
    ValueList<std::string> units = mDisplay.ValueUnits();
    if (inConfig.Get(CfgID::ValueUnits, units))
        mDisplay.SetValueUnits(units);
...
void VisDisplayGraph::OnSignal(const GenericSignal &s)
{
    ...
    int curChannels = mNumChannels, curSamples = mDisplay.NumSamples();
    ...
    if (mAutoScale)
    {
        int autoScaleSamples = mDisplay.NumSamples();
        const PhysicalUnit& unit = decimatedSignal.Properties().ElementUnit();
        if (unit.Symbol() == "s")
            autoScaleSamples = unit.PhysicalToRaw(mAutoScaleTime);
        autoScaleSamples = std::min(mDisplay.NumSamples(), autoScaleSamples);
        double sigMax = -Infinity<double>(), sigMin = Infinity<double>();
        for(int ch = 0; ch < mDisplay.NumChannels(); ++ch)
        {
            for (int sample = 0; sample < autoScaleSamples; ++sample)
            {
                sigMax = std::max(sigMax, mDisplay.Data(ch, sample));
                sigMin = std::min(sigMin, mDisplay.Data(ch, sample));
            }
        }
        if (sigMin > 0)
            sigMin = 0;
...
void VisDisplayGraph::BuildContextMenu()
{
...
{
    mDisplay.SetDisplayMode((mDisplay.DisplayMode() + 1) % SignalDisplay::numDisplayModes);
}
...
{
    return mDisplay.DisplayMode() == SignalDisplay::polyline || mDisplay.DisplayMode() == SignalDisplay::field2d;
}
...
void VisDisplayGraph::ChooseColors()
{
    // The dialog's "custom colors" are used to hold channel colors.
    ColorList channelColors = mDisplay.ChannelColors();
    ColorListChooser().SetPrompt("Choose channel colors").Display(channelColors);
    mDisplay.SetChannelColors(channelColors);
    Visconfigs()[mVisID].Put(CfgID::ChannelColors, channelColors, UserDefined);
}

bool VisDisplayGraph::ChooseColors_Enabled() const
{
    return mDisplay.ColorDisplay() && mDisplay.DisplayMode() == SignalDisplay::polyline;
}
...
void VisDisplayGraph::FewerChannels()
{
    ...
    int n = mDisplay.DisplayGroups(), roundedN = 1;
    while (n >>= 1)
        roundedN <<= 1;
...
void VisDisplayGraph::MoreChannels()
{
    int maxDisplayGroups = mNumChannels / mDisplay.ChannelGroupSize(),
        newDisplayGroups = std::min(maxDisplayGroups, mDisplay.DisplayGroups() * 2);
...
double VisDisplayGraph::FilterCaptionToValue(const char *inCaption) const
{
    ...
    while (*p && !::isdigit(*p))
        ++p;
    return ::atof(p) * NominalUnitsPerSample() * FilterUnitToValue(mDisplay.SampleUnit());
}
...
void VisDisplayGraph::SyncDisplay()
{
    int heightReduction = 0;
    if (mpStatusBar->isVisible())
        heightReduction = mpStatusBar->height();

    GUI::DrawContext dc = {{0, 0, float(this->width()), float(this->height()) - heightReduction}, 0};
    dc.device = this;
    mDisplay.SetContext(dc);
}

void VisDisplayGraph::paintEvent(QPaintEvent *iopEvent)
{
