//////////////////////////////////////////////////////////////////////
// $Id: MainWindow.cpp 6562 2022-02-12 16:32:17Z mellinger $
      ...
      mUpdateTimerID(0), mState(0)
{
    ...
    if (!mWebsocket.isEmpty())
        OperatorModule::WebsocketListen(mWebsocket.toLocal8Bit().constData());
...
void MainWindow::ReadCommandLine()
{
    ...
    while (i < qApp->arguments().size())
    {
        ...
        {
            if ((i + 1) < qApp->arguments().size() && !qApp->arguments().at(i + 1).startsWith("-"))
                mWebsocket = qApp->arguments().at(++i);
            else
                mWebsocket = "localhost:3998";
        }
...
void MainWindow::UpdateDisplay()
{
    ...
    QString windowCaption = TXT_WINDOW_CAPTION " ", statusText = "N/A";
...
void MainWindow::SetFunctionButtons()
{
    ...
    {
        QString name = gpPreferences->mButtons[i].Name, script = gpPreferences->mButtons[i].Cmd;
...
#define EVENT(x) {Preferences::x, #x},
        EVENT(OnConnect) EVENT(OnSetConfig) EVENT(OnResume) EVENT(OnSuspend) EVENT(OnStart)
#undef EVENT
    };
...
{
    QMetaObject::invokeMethod(mpSyslog, "AddEntry", Qt::AutoConnection, Q_ARG(QString, s),
...
bool MainWindow::OnUnknownCommand(const char *inCommand)
{
    bool result = false;
    ...
    if (!::stricmp(verb.c_str(), "Show"))
        ...
        action = set;
    else if (!::stricmp(verb.c_str(), "Visualize"))
        action = visualize;
    if (action != unknown && !::stricmp(type.c_str(), "Window"))
    {
        ...
        if (name.empty() || !::stricmp(name.c_str(), "Main"))
            ...
            object = config;
        else if (!::stricmp(name.c_str(), "Watches"))
            object = watches;
        else
            result = false;
        ...
        {
        case main:
            QMetaObject::invokeMethod(this, "setVisible", Qt::AutoConnection, Q_ARG(bool, action == show));
            break;
        case log:
            QMetaObject::invokeMethod(mpSyslog, "setVisible", Qt::AutoConnection, Q_ARG(bool, action == show));
            break;
        case watches:
            QMetaObject::invokeMethod(WatchesWindow::Instance(), "setVisible", Qt::AutoConnection,
        ...
        case config: {
            ...
            {
            case show:
                QMetaObject::invokeMethod(this, "on_pushButton_Config_clicked", Qt::AutoConnection);
                break;
            case hide:
                QMetaObject::invokeMethod(gpConfig, "close", Qt::AutoConnection);
                break;
                break;
            default:
                result = false;
    ...
    else if (action == set && !::stricmp(type.c_str(), "Button"))
    {
        ...
        if (idx >= 0 && idx < Preferences::numButtons)
        {
            ...
            iss >> caption >> script;
            QMetaObject::invokeMethod(this, "SetFunctionButton", Qt::AutoConnection,
    ...
    else if (action == visualize && !::stricmp(type.c_str(), "Watch") && !name.empty())
    {
        result = true;
        ParserToken token;
        int decimation = 1;
        if (!::stricmp(name.c_str(), "Decimate")) {
            iss >> decimation;
            iss >> name;
        }
        QString list = name.c_str();
        while (iss >> token)
            list += QString(", ") + token.c_str();
        if (!token.empty())
            list = "{ " + list + " }";
        QMetaObject::invokeMethod(WatchesWindow::Instance(), "AddWatch", Qt::AutoConnection, Q_ARG(int, decimation), Q_ARG(QString, list));
    }
    return result;
}
...
const char *MainWindow::OnScriptHelp()
{
    return "Show Window <name>, Hide Window <name>, Set Title <window title>, "
           ...
           "Visualize Watch [decimate <x>] <expr>, "
           "names are: Main, Log, Configuration, Watches";
}
...
void MainWindow::OnVisBitmap(const char *visID, int width, int height, unsigned short *data)
{
    VisDisplay::HandleBitmap(visID, BitmapImage(width, height, data));
}

void MainWindow::OnVisPropertyMessage(const char *visID, int cfgID, const char *value)
{
