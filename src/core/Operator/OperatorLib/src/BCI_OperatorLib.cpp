///////////////////////////////////////////////////////////////////////////
// $Id: BCI_OperatorLib.cpp 6644 2022-03-29 17:05:55Z mellinger $
...
static TelnetServer *spTelnetServer;
static WebsocketServer *spWebsocketServer;
...
DLLEXPORT int STDCALL BCI_PutEvent(const char *inEventLine)
{
    ...
    if (!event.ExtractFrom(iss))
        return 0;
    WithLock(gpStateMachine) gpStateMachine.load()->Events().Add(event);
    return 1;
}
...
DLLEXPORT const char *STDCALL BCI_GetEvent(long inIndex)
{
    ...
    if (!result && inIndex >= 0 && inIndex < gpStateMachine.load()->Events().Size())
    {
        std::ostringstream oss;
        oss << gpStateMachine.load()->Events().ByIndex(inIndex);
        result = AllocateCopy(oss.str().c_str());
    }
    return result;
}
...
DLLEXPORT int STDCALL BCI_WebsocketListen(const char *inAddress)
{
    ...
    spWebsocketServer = new WebsocketServer(*gpStateMachine, pAddress);
    return spWebsocketServer != 0;
}

/*
function:  BCI_WebsocketClose
purpose:   Stop the websocket server, closing any open connections.
arguments: <n/a>
returns:   1 if no error occurred, 0 otherwise.
*/
DLLEXPORT int STDCALL BCI_WebsocketClose()
{
    WebsocketServer *p = spWebsocketServer;
    spWebsocketServer = 0;
    delete p;
    return 1;
}
