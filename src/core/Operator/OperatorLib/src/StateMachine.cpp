////////////////////////////////////////////////////////////////////////////////
// $Id: StateMachine.cpp 6644 2022-03-29 17:05:55Z mellinger $
...
StateMachine::WatchDataLock::~WatchDataLock()
{
  StateMachine& s = (*this)();
  int sampleBlockSize = s.mStateVector.Samples() - 1;
  double blockDuration = 1.0 / s.mControlSignal.Properties().UpdateRate();
  PrecisionTime sourceTime = s.mStateVector.StateValue("SourceTime");
  s.Watches().Check(sampleBlockSize, blockDuration, sourceTime);
}
...
StateMachine::~StateMachine()
{
    ...
    WithLock(mListeners) for (Listeners::iterator i = mListeners.begin(); i != mListeners.end(); ++i)(*i)->Abort();
    ...
    while (!empty)
    {
        WithLock(mListeners) empty = mListeners.empty();
        if (!empty)
            ThreadUtils::SleepForMs(1);
    }
...
std::string StateMachine::SuggestUDPAddress(const std::string &inAddressHint) const
{
    ...
    std::string result, local = socket.address(), local2, hint = inAddressHint.empty() ? local : inAddressHint;
...
void StateMachine::LogMessage(int inCallbackID, const std::string &inMessage)
{
    ...
    WithLock(mListeners) for (Listeners::iterator i = mListeners.begin(); i != mListeners.end(); ++i)(*i)
        ->HandleLogMessage(inCallbackID, inMessage);
}

void StateMachine::TriggerEvent(int inCallbackID)
{
