...
void ComplexVisualizationDemoFilter::Private::createVisualizations(const SignalProperties &Input, int maxWindows)
{
    ...
    for (int ch1 = 0; ch1 < Input.Channels(); ++ch1)
    {
        for (int ch2 = ch1; ch2 < Input.Channels(); ++ch2)
        {
            ...
            // Choose window positions such that windows form (a triangular part of) a matrix.
...
ComplexVisualizationDemoFilter::Private::VisualizationObject::VisualizationObject(const Private *p,
    ...
    : p(p), mVis(visID), mCallAsyncReset(&VisualizationObject::asyncReset, this),
...
void ComplexVisualizationDemoFilter::Private::VisualizationObject::setPosition(const GUI::Rect &pos)
{
#if USE_QT_PAINTER
...
void ComplexVisualizationDemoFilter::Private::VisualizationObject::asyncReset()
{
