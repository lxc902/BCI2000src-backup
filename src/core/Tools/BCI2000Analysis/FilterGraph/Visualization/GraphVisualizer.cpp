...
struct Visualizer::Private
{
    ...
    // using dynamic_cast<>(). Using the Vistor pattern would
    ...
    static Vis visualizeApplicationNode(ApplicationNode *, Graph::iterator);
...
Vis Visualizer::Private::visualizeNode(Graph::iterator i)
{
    ...
    auto pApplicationNode = dynamic_cast<ApplicationNode *>(*i);
    if (pApplicationNode)
...
Vis Visualizer::Private::visualizeNode(Node *, Graph::iterator i)
{
    Vis items;
...
Vis Visualizer::Private::visualizeApplicationNode(ApplicationNode *p, Graph::iterator i)
{
    auto vis = visualizeNode(static_cast<Node *>(p), i);
    Assert(!vis.first.empty());
    vis.first.front()->setColor(Qt::green);
