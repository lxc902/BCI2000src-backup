...
#define _(x) {#x, (void **)&x, Dylib::Import::cMangled},
            _(InterfaceVersion) _(Compiler) _(BuildType) _(BuildInfo) _(FilterName) _(ReleaseString) _(SetBcierr)
                _(SetBciwarn) _(SetBciout) _(SetBcidbg) _(SetVisualizationChannel) _(SetMeasurementUnits)
                    _(SetEnvironmentContext) _(Instantiate)
#undef _
        };
