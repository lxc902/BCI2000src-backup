...
// Description: Data window class for BCI2000Analysis.
//
...
struct DataWindow::Private : QObject
{
    ...
    GenericSignal mRsq, mMean1, mMean2;
...
DataWindow::Private::Private(QMainWindow *p) : QObject(p), mpParent(p)
{
}

void DataWindow::Private::setAnalysisData(const AnalysisData &data)
{
    mAnalysisData = data;
    ...
    auto pWidget = new EditMenu::CopyWidget(mpParent);
    auto pLayout = new QHBoxLayout;
    pLayout->setContentsMargins(0, 0, 0, 0);
    pWidget->setLayout(pLayout);
    auto pGroup = new QGroupBox(QString("Training data r^2 map (N1=%1 N2=%2 dim=%3)").arg(n1).arg(n2).arg(ndim));
    pWidget->layout()->addWidget(pGroup);
    auto pLayout2 = new QVBoxLayout;
    pLayout2->setContentsMargins(0, 0, 0, 0);
    pGroup->setLayout(pLayout2);
    auto pSignalWidget = new SignalWidget(mRsq, mpParent);
...
void DataWindow::Private::onSaveAnalysisData()
{
...
void DataWindow::Private::onShowCurvesAtChannel()
{
...
void DataWindow::Private::onShowDistributionAtElement()
{
...
void DataWindow::Private::onShowCurvesAt(int channel)
{
    auto pWindow = new ChannelPlotsWindow(mMean1, mMean2, mRsq, channel, mpParent);
    pWindow->show();
}

void DataWindow::Private::onShowDistributionAt(int element)
{
    auto pWindow = new TopoplotWindow(mRsq, element, mpParent);
    pWindow->resize(400, 400);
    pWindow->show();
}

void DataWindow::Private::train()
{
    ...
    QProgressDialog progress(QString("Training ") + pClassifier->method() + " classifier...", "Cancel", 0, totalCount,
                             mpParent);
    ...
    {
        QMessageBox::critical(mpParent, "Classifier training error", QString("Classifier training: ") + e.what());
        canceled = true;
    }
    ...
    if (!canceled)
    {
        auto pWindow = new ClassifierWindow(mAnalysisData, *pClassifier);
...
DataWindow::DataWindow(const AnalysisData &data) : QMainWindow(nullptr), p(new Private(this))
{
    setAttribute(Qt::WA_DeleteOnClose, true);

    static int count = 0;
    setWindowTitle(QString("Analysis Data %1").arg(++count));
    ...
    pMenu->addSeparator();
    p->mpTrainClassifier1 =
        pMenu->addAction("&Train Classifier...", p, &Private::train, QKeySequence(Qt::CTRL + Qt::Key_T));
