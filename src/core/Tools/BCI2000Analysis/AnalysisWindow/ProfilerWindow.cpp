...
struct ProfilerWindow::Private : QObject
{
    ...
    Private(QWidget *parent) : QObject(parent), mpParent(parent)
    {
    }
...
{
    QString fileName = FileDialog::getSaveFileName(mpParent, "Save profiler report", "Text files (*.txt)", "output",
                                                   "ProfilerReport.txt");
    if (!fileName.isEmpty())
    {
        ...
        writeToStream(file);
        if (!file)
            QMessageBox::critical(mpParent, "Error", "Could not write to file\n" + fileName);
    }
}
...
{
    static int count = 0;
    setWindowTitle(QString("Profiler Results %1").arg(++count));
