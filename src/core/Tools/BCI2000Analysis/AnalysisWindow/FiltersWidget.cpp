...
struct FiltersWidget::Private : QGraphicsScene
{
    ...
    bool mEditable;
    FiltersWidget *mpParent;

    Private(FiltersWidget *parent) : QGraphicsScene(parent), mpParent(parent), mEditable(false)
    {
    }
    ...
    void onCustomContextMenuRequested(const QPoint &where)
    {
        if (!mEditable)
            return;
        auto pItem = mpParent->itemAt(where);
        ...
        QMenu menu;
        QAction *pAction = menu.addAction("Delete", this, [pFilterItem, this] { onDelete(pFilterItem); });
        bool isStatic = true;
        if (pFilterItem)
        {
        ...
        pAction->setEnabled(pConnectorItem);
        menu.exec(mpParent->mapToGlobal(where));
    }
    ...
    void onDelete(FilterGraphicsItem *pItem)
    {
        ...
        if (pSplitter)
        {
            ...
            QString text = QString("Really delete splitter \"%1\" and its subchains \"%2\" and \"%3\"?")
                               ...
                               .arg(sub1)
                               .arg(sub2);
            int button = QMessageBox::question(mpParent, "Delete splitter...", text);
    ...
    void onRename(FilterGraphicsItem *pItem)
    {
        ...
        bool ok = false;
        QString newName = QInputDialog::getText(mpParent, title, label, QLineEdit::Normal, pItem->text(), &ok,
    ...
    void onInsertFilter(ConnectorGraphicsItem *pItem)
    {
        ...
        bool ok = false;
        auto name = QInputDialog::getItem(mpParent, "Insert Filter...", "Filter type:", names, 0, false, &ok,
                                          Qt::MSWindowsFixedSizeDialogHint);
        if (ok)
        {
            ...
            {
                QString fileName =
                    FileDialog::getOpenFileName(mpParent, "Choose a BCI2000 filter DLL", "Shared libraries (*.dll)");
                if (!fileName.isEmpty())
                {
                    ...
                    else
                    {
                        ...
                        QMessageBox::critical(mpParent, "Filter DLL error", err);
