...
void SignalWidget::contextMenuEvent(QContextMenuEvent *ev)
{
    ...
    {
        QString text = "Show &Curve(s) at channel " + QString::fromUtf8(p->mSignalProperties.ChannelLabel(ch).c_str());
    ...
    if (s >= 0 && s < p->mDisplay.NumSamples())
    {
        ...
        QString text = "Show &Distribution at " + QString::fromUtf8(label.c_str());
