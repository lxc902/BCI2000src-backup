////////////////////////////////////////////////////////////////////////////////
// $Id: MainWindow.cpp 6644 2022-03-29 17:05:55Z mellinger $
...
QStringList MainWindow::ScanForPrograms()
{
    ...
    for (bci::Inventory::mapped_type::const_iterator i = mInventory[bci::Inventory::Operator].begin();
         i != mInventory[bci::Inventory::Operator].end(); ++i)
        programs.removeOne(QString::fromLocal8Bit(i->c_str()));
    // Remove all tools.
    for (bci::Inventory::mapped_type::const_iterator i = mInventory[bci::Inventory::Tool].begin();
         i != mInventory[bci::Inventory::Tool].end(); ++i)
        programs.removeOne(QString::fromLocal8Bit(i->c_str()));
    // Remove all helpers.
    for (bci::Inventory::mapped_type::const_iterator i = mInventory[bci::Inventory::Helper].begin();
      i != mInventory[bci::Inventory::Helper].end(); ++i)
      programs.removeOne(QString::fromLocal8Bit(i->c_str()));
    return programs;
}
...
{
    std::string progdir = std::string(QApplication::applicationDirPath().toLocal8Bit().constData()) + "/";
    ...
    if (mParameterFiles.size() > 0)
    {
        ...
        for (int i = 0; i < mParameterFiles.size(); ++i)
            command << "LOAD PARAMETERFILE " << EncodedString(mParameterFiles.at(i).toLocal8Bit().constData()) << "; ";
        command << " SETCONFIG;\"";
    }
...
bool MainWindow::ExecuteCommand(const char *inCommand, const char *inWorkingDir)
{
#if _WIN32
    STARTUPINFOA si;
    ...
    BOOL result = ::CreateProcessA(NULL, command, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, inWorkingDir, &si, &pi);
    delete[] command;
    if (result)
    {
...
QStringList MainWindow::ParseCommand(const char *inCommand)
{
    QStringList result;
    ...
    while (*begin != '\0')
    {
        ...
        result.push_back(std::string(begin, end).c_str());
        begin = ++end;
        while (*begin == ' ' && *begin != '\0')
            ++begin;
    }
