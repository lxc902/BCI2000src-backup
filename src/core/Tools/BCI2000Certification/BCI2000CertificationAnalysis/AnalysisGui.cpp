////////////////////////////////////////////////////////////////////////////////
// $Id: AnalysisGui.cpp 6484 2022-01-03 16:59:03Z mellinger $
...
struct AnalysisGui::FilesListField : CollapsibleGroup, Broadcaster
{
    ...
    FilesListField(QString title) : CollapsibleGroup(title)
    {
        ...
        Connect(mpAddFilesButton, SIGNAL(clicked()),
                MemberCall<void(FilesListField *)>(&FilesListField::onAddFilesButtonClicked, this));
        Connect(mpRemoveFilesButton, SIGNAL(clicked()),
                MemberCall<void(FilesListField *)>(&FilesListField::onRemoveFilesButtonClicked, this));
        Connect(mpFilesList, SIGNAL(itemSelectionChanged()),
                MemberCall<void(FilesListField *)>(&FilesListField::onFilesListItemSelectionChanged, this));
        mpFilesList->setContextMenuPolicy(Qt::CustomContextMenu);
        Connect(mpFilesList, SIGNAL(customContextMenuRequested(const QPoint&)),
                MemberCall<void(FilesListField *)>(&FilesListField::onFilesListCustomContextMenuRequested, this));
    }
    ...
    void onFilesListCustomContextMenuRequested()
    {
        ...
        if (pItem)
        {
            ...
            std::string executable, args;
            if (pResult)
            {
                args = "\"";
                args += pItem->text().toLocal8Bit().constData();
                args += "\"";
            }
...
{
    FileSelectorField(const QString &title) : CollapsibleGroup(title)
    {
...
{
    ScriptField(const QString &title) : CollapsibleGroup(title)
    {
        auto pLayout = new QVBoxLayout;
