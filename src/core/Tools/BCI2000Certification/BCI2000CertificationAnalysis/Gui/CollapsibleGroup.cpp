...
CollapsibleGroup::CollapsibleGroup(const QString &title)
    : mpToolButton(nullptr), mpCheckBox(nullptr), mpLayout(new QGridLayout), mpLabel(new ClickableLabel(title)),
...
void CollapsibleGroup::setUsingCheckBox(bool b)
{
    ...
    if (b && !mpCheckBox)
    {
        ...
        Connect(mpCheckBox, SIGNAL(toggled(bool)),
                MemberCall<void(CollapsibleGroup *)>(&CollapsibleGroup::onCollapsedToggled, this));
        onCollapsedToggled();
    }
    else if (!b && !mpToolButton)
    {
        ...
        Connect(mpToolButton, SIGNAL(toggled(bool)),
                MemberCall<void(CollapsibleGroup *)>(&CollapsibleGroup::onCollapsedToggled, this));
        onCollapsedToggled();
    }
}
