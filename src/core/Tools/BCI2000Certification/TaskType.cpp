////////////////////////////////////////////////////////////////////////////////
// $Id: TaskType.cpp 6484 2022-01-03 16:59:03Z mellinger $
...
{
    String s = "AmpState:=state('SourceTime');\n";
    if (AudStateName.empty())
        s += "AudState:=stream(\"\");\n";
    else
...
TaskType *Config::taskByName(const String &name)
{
...
void Config::validateTaskName(const String *pName)
{
    Tasks.revalidateLater();
}

void Config::validateRootDir(const String *dir)
{
    if (!dir || dir->empty())
...
void Config::validateDataDir(const String *dir)
{
    if (!dir || dir->empty())
        validationError() << "must specify a directory";
    else if (!FileUtils::IsDirectory(*dir + "/.."))
...
bool Config::isValidExecutable(const String &file) const
{
...
void Config::validateExecutable(const String *file)
{
    if (file && !file->empty())
    {
...
void Config::validateStream(const String* pName)
{
    String prevVal;
