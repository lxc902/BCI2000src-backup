...

{
    ...
    {
        ap::vmove(data.getrow(static_cast<int>(i + target.size()), 0, col_Data - 1),
    ...
    params.weights.setbounds(0, static_cast<int>(var_ind.size() * DecFact) - 1, 0, 2);
    ...
    params.channels.setbounds(0, static_cast<int>(chused.size()) - 1);
    // Cristhian modification April 17, 2010
    params.chSet.setbounds(0, static_cast<int>(chused.size()) - 1);
