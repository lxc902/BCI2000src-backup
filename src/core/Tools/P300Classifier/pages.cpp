    ...
    AnalysisMode& mode)
{
    ...
    for (int file = 0; file < pFilesList->count(); file++)
    {
        ...
        if (ApplicationFilterChain[file] == Experiment[1])
        {
            ...
            {
                QMessageBox msgBox(QMessageBox::Warning, tr("Warning"), errors.str().c_str(), QMessageBox::NoButton, this);
                msgBox.addButton(tr("&Continue"), QMessageBox::AcceptRole);
                if (msgBox.exec() == QMessageBox::AcceptRole)
                {
                    pFilesList->item(file)->setBackground(QColor("lightpink"));
...
void WeightsWorkerThread::GenerateOrApplyFeatureWeights(bool generate)
{
    ...
    if (generate)
    {
        ...
        if (spDataPage->mpWritePRMLineEdit->text().length() == 0)
        {
            size_t found;
            std::string filedata = spDataPage->mTrainingDataFiles[0];
            ...
            spDataPage->mpWritePRMLineEdit->setText(fileSug.c_str());
        }
        spDataPage->mFeatureWeightsGenerated = true;
        if (spDataPage->mAutoWrite)
        {
            if (spDataPage->WriteParameterFragment(false))
            {
