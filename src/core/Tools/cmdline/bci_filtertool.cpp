////////////////////////////////////////////////////////////////////
// $Id: bci_filtertool.cpp 6644 2022-03-29 17:05:55Z mellinger $
...
bool FilterWrapper::OnVisSignal(std::istream &is)
{
    ...
    if (s.Unserialize(is) && s.VisID().empty())
    {
        ...
        case Environment::nonaccess: {
            ...
            {
                std::string name = filterParams.ByIndex(i).Name();
                std::string path = filterParams.ByIndex(i).Path();
                if (mParamlist.Exists(name))
                {
                    filterParams.ByIndex(i).AssignValues(mParamlist.ByPath(name));
                    mParamlist.Delete(name);
                }
                mParamlist.ByPath(path) = filterParams.ByIndex(i);
            }
        }
            /* no break */
