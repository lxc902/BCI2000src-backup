////////////////////////////////////////////////////////////////////////////////
// $Id: SignalDisplay.cpp 6655 2022-04-01 17:50:10Z mellinger $
...
SignalDisplay::SignalDisplay()
    : mDataWidth(0), mDataHeight(0), mDataLeft(cLabelWidth), mLabelWidth(cLabelWidth), mMarkerHeight(0),
      ...
      mNumDisplayChannels(0), mTopGroup(0), mChannelGroupSize(1), mMarkerChannels(0), mTickLength(mTickLength),
      mFontSize(11), mNumericValueWidth(0), mMinValue(cMinValueDefault), mMaxValue(cMaxValueDefault),
      mAxisColor(cAxisColorDefault), mChannelColors(cChannelColorsDefault), mParent(NULL), mpSignalPoints(NULL),
...
SignalDisplay &SignalDisplay::Invalidate()
{
    ...
    if (inSignal.Channels() != mData.Channels())
        WithLock(mDataLock)
        {
            ...
            if (mNumDisplayGroups == 0)
                mNumDisplayGroups = std::min<int>(newNumDisplayGroups, cInitialMaxDisplayGroups);
...
SignalDisplay &SignalDisplay::SetDisplayGroups(int inDisplayGroups)
{
    ...
    mNumDisplayChannels = std::min(numSignalChannels, mNumDisplayGroups * mChannelGroupSize);
...
int SignalDisplay::PosToGroup(int p)
{
    if (p < GroupTop(0))
        return -1;
...
void SignalDisplay::SetupPainting(PaintInfo &p, const void *inUpdateRgn)
{
    SyncGraphics();
...
void SignalDisplay::DrawSignalPolyline(const PaintInfo &p)
{
    ...
    if (mShowBaselines)
    {
        ...
        {
            baselinePoints[0].setY(ToInt(ChannelBottom(i) + (baseInterval * mMinValue) / (mMaxValue - mMinValue)));
    ...
    {
        int channelBottom = ChannelBottom(i), channel = i + mTopGroup * mChannelGroupSize;
...
void SignalDisplay::DrawSignalField2d(const PaintInfo &p)
{
    ...
    for (int i = 0; i < mNumDisplayChannels; ++i)
    {
        for (int j = sampleBegin; j < sampleEnd; ++j)
        {
            ...
            QRect dotRect(SampleLeft(j), ChannelTop(i), SampleRight(j) - SampleLeft(j),
...
void SignalDisplay::DrawCursor(const PaintInfo &p)
{
    if (mShowCursor)
    {
        ...
        QRect cursorRect(SampleLeft(cursorSample) - p.cursorWidth, 0, p.cursorWidth,
...
void SignalDisplay::DrawMarkers(const PaintInfo &p)
{
    ...
    for (size_t i = 0; i < mXAxisMarkers.size(); ++i)
    {
        ...
        QRect markerBar(markerX - cAxisWidth / 2, p.axisY - 4 * cAxisWidth, cAxisWidth, 4 * cAxisWidth - 1);
...
void SignalDisplay::DrawValueUnit(const PaintInfo &p)
{
    ...
    {
        int ch = mNumDisplayChannels + mTopGroup * mChannelGroupSize - 1;
        const PhysicalUnit& unit = mValueUnits.size() > ch ? mValueUnits[ch] : mValueUnit;
        p.painter->setFont(p.labelFont);
        ...
        double unitsPerPixel = ::fabs((mMaxValue - mMinValue) * unit.Gain() / baseInterval),
        ...
        if (mMinValue == 0)
        {
            int left = SampleLeft(0) + mTickLength, top = ChannelBottom(0) - pixelLength;
