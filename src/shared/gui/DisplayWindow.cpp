////////////////////////////////////////////////////////////////////////////////
// $Id: DisplayWindow.cpp 6698 2022-04-26 16:54:55Z mellinger $
...
class GuiWindow : QWindow
{
  public:
    ...
    GuiWindow(GUI::DisplayWindow &inParent) : mrParent(inParent)
    {
...
struct DisplayWindow::Data
{
    ...
    std::atomic<int> mUpdateRateDecimation{ false };
    ...
    struct RenderingThread : Thread
    {
        ...
        RenderingThread(DisplayWindow::Data *d) : Thread("Rendering Thread"), d(d)
        {
        }
        int OnExecute()
        {
...
DisplayWindow::Data::Data(DisplayWindow *pSelf)
    : mpSelf(pSelf), mTitle(FileUtils::ApplicationTitle()), mLeft(0), mTop(0), mWidth(0), mHeight(0), mVisible(false),
...
bool DisplayWindow::Data::Adjust(GUI::DrawContext &dc)
{
    bool sizeChanged = false;
    float devicePixelRatio = mpWindow->devicePixelRatio();
    if (mWidth * devicePixelRatio != dc.rect.Width() || mHeight * devicePixelRatio != dc.rect.Height())
    {
        dc.rect.left = 0;
        dc.rect.top = 0;
        dc.rect.right = mWidth * devicePixelRatio;
        dc.rect.bottom = mHeight * devicePixelRatio;
        sizeChanged = true;
    }
...
void DisplayWindow::Data::Update(void *p)
{
    mRenderingQueue.Push(RenderState(-1, TimeUtils::MonotonicTime(), p));
}

int DisplayWindow::Data::RenderingThreadFunc()
{
    ...
    WithThreadTask(task, ThreadUtils::Priority::Maximum)
    {
        ...
        while (!mStopRendering)
        {
            ...
            WithGLContext(mpWindowGL) do
            {
                ...
                float ratio = mpWindow->devicePixelRatio();
                mpWindowGL->SetViewport(0, 0, mWidth * ratio, mHeight * ratio);
...
DisplayWindow &DisplayWindow::Data::OnChange()
{
...
// DisplayWindow
DisplayWindow::DisplayWindow() : d(new Data(this))
{
}
...
#define PROPERTY_DEF(name, type, apply)                                                                                \
    DisplayWindow &DisplayWindow::Set##name(type value)                                                                \
    {                                                                                                                  \
        d->m##name = value;                                                                                            \
        return apply;                                                                                                  \
    }                                                                                                                  \
    type DisplayWindow::name() const                                                                                   \
    {                                                                                                                  \
        return d->m##name;                                                                                             \
    }
...
DisplayWindow &DisplayWindow::Update(void *p)
{
    d->Update(p);
    return *this;
}

ClockTick DisplayWindow::CurrentFrame() const
{
    return *d->mFrame;
}

ClockTick::Statistics DisplayWindow::FrameClockStatistics() const
{
...
DisplayWindow &DisplayWindow::ConfigureBitmapBuffer(int inWidthPx, int inHeightPx)
{
...
void DisplayWindow::OnBitmapData(GUI::Bitmap &ioImage, const GUI::Rect *inpRectPx) const
{
    OffscreenBuffer &b = d->mScaledContentBuffer;
    if (b.Valid())
