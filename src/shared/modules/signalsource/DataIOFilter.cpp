////////////////////////////////////////////////////////////////////////////////
// $Id: DataIOFilter.cpp 6640 2022-03-23 19:29:19Z mellinger $
...
DataIOFilter::DataIOFilter()
    : mpADC(GenericFilter::PassFilter<GenericADC>()), mpSourceFilter(NULL), mpFileWriter(NULL), mpFileWriterInput(NULL),
      ...
      mSampleBlockSize(0), mPrevStimulusTime(0), mBCIEvents(0), mWasResting(false), p(new Private)
{
...
void DataIOFilter::Initialize(const SignalProperties & /*Input*/, const SignalProperties &Output)
{
    ...
    if (Parameter("VisualizeSourceDecimation") == "auto")
    {
        ...
        bcidbg(5) << "Choosing a VisualizeSourceDecimation value of " << mVisualizeSourceDecimation;
    }
    else
    {
...
void DataIOFilter::ProcessBCIEvents()
{
    ...
    while (!mBCIEvents.IsEmpty() && PrecisionTime::SignedDiff(mBCIEvents.FrontTimeStamp(), sourceTime) < 0)
    {
        int offset = std::round(
            ((mBlockDuration - PrecisionTime::UnsignedDiff(sourceTime, mBCIEvents.FrontTimeStamp())) *
             mSampleBlockSize) /
            mBlockDuration);
        std::istringstream iss(mBCIEvents.FrontDescriptor());
        std::string name;
        iss >> name;
        ...
        { // Set the state at a single position only.
            ...
            while (offset < mSampleBlockSize &&
                   Statevector->StateValue(state.Location(), state.Length(), offset) != val)
                ++offset;
