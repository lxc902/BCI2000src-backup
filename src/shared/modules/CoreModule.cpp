////////////////////////////////////////////////////////////////////////////////
// $Id: CoreModule.cpp 6745 2022-05-17 15:41:07Z mellinger $
...
CoreModule::CoreModule()
    : mFiltersInitialized(false), mTerminating(false), mRunning(false), mActiveResting(false), mStartRunPending(false),
      mStopRunPending(false), mNeedStopRun(false), mReceivingNextModuleInfo(false), mGlobalID(NULL),
...
bool CoreModule::Initialize(int &ioArgc, char **ioArgv)
{
    ...
    bool executeTests = false, printVersion = false, printHelp = false;
    int i = 1;
    while (i < ioArgc)
    {
        ...
        else if (curArg.find("--") == 0)
        {
            ...
            Param param("/" + paramName, "System:Command Line Arguments", "variant", paramValue, paramValue, paramValue,
                        paramValue);
...
void CoreModule::InitializeOperatorConnection(const std::string &inOperatorAddress)
{
    ...
    mParamlist.ByPath("/OperatorAddress").Value() = String() << inOperatorAddress;
    ...
    mParamlist.ByPath("/ListeningAddress").Value() = String() << mServerSocket.IP() << ":" << mServerSocket.Port();
    ...
    mParamlist.ByPath("/OperatorIP").Value() = mOperatorSocket.IP();
...
void CoreModule::AutoConfigFilters()
{
    for (int i = 0; i < mParamlist.Size(); ++i)
        mParamlist.ByIndex(i).Unchanged();
    ...
    {
        const Param &r = restoreParams.ByIndex(i);
        Param &p = mParamlist.ByPath(r.Path());
        if (p.Changed())
        {
            p = r;
            p.Unchanged();
    ...
    if (mParamlist.Exists("/DebugLevel") && ::atoi(mParamlist.ByPath("/DebugLevel").Value().c_str()))
    {
      ...
      for (int i = 0; i < mParamlist.Size(); ++i)
            if (mParamlist.ByIndex(i).Changed())
                bciout << "AutoConfig: " << mParamlist.ByIndex(i);
    }
