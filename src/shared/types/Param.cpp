////////////////////////////////////////////////////////////////////////////////
// $Id: Param.cpp 6549 2022-02-10 14:54:48Z mellinger $
...
Param &Param::SetDimensions(size_t inRows, size_t inCols)
{
    ...
    if (newCols > cols)
        for (size_t i = 0; i < rows; ++i)
            mValues.insert(mValues.begin() + i * newCols + cols, newCols - cols, sDefaultValue);
    else
        for (size_t i = 0; i < rows; ++i)
            mValues.erase(mValues.begin() + (i + 1) * newCols, mValues.begin() + i * newCols + cols);
...
// Returns:    N/A
// **************************************************************************
Param::Param() : mReadonly(false), mAllowOverride(false), mChanged(false), mParentPath("/"), mValues(1, sDefaultValue)
{
}
...
{
    if (inN > 1 && mType.find("list") == mType.npos && mType.find("matrix") == mType.npos)
        mType = "list";
...
// Returns:    N/A
// **************************************************************************
const String &Param::ParamValue::AsString() const
{
    if (!mpString)
