////////////////////////////////////////////////////////////////////////////////
// $Id: GenericSignal.cpp 6484 2022-01-03 16:59:03Z mellinger $
...
{
    ArrayMemory(const std::shared_ptr<SharedMemory> &p, size_t count)
        : Memory(reinterpret_cast<ValueType *>(p->Memory()), count), mpShm(p)
    {
    }
...
struct ShmPool_ : private std::map<std::string, ShmPtr>
{
...
GenericSignal &GenericSignal::SetProperties(const SignalProperties &inSp)
{
    if (inSp.Channels() != mProperties.Channels() || inSp.Elements() != mProperties.Elements())
    {
        ...
        if (mSharedMemory)
        {
            ...
            std::shared_ptr<Array::Memory> p(new ArrayMemory(mSharedMemory, newSize));
            newValues = Array(p);
        }
        else
...
bool GenericSignal::ShareAcrossModules()
{
    if (!mSharedMemory && mValues.Count() != 0)
    {
        ...
        std::shared_ptr<Array::Memory> p(new ArrayMemory(mSharedMemory, mValues.Count()));
        Array newValues(p);
