////////////////////////////////////////////////////////////////////////////////
// $Id: ParamList.cpp 6550 2022-02-10 15:49:39Z mellinger $
...
bool ParamList::Load(const std::string &inFileName, bool inImportNonexisting)
{
    ...
    if (!inImportNonexisting)
        for (int i = 0; i < paramsFromFile.Size(); ++i)
            if (!Exists(paramsFromFile.ByIndex(i).Path()))
                unwantedParams.insert(paramsFromFile.ByIndex(i).Path());
    ...
    for (int i = 0; i < paramsFromFile.Size(); ++i)
    {
        Param &p = paramsFromFile.ByIndex(i);
        if (Exists(p.Path()))
        {
