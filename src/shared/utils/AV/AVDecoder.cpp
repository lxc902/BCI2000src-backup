...
{
    AVUtils::Log log;
    int flags = 0; // not AVSEEK_FLAG_ANY, as we want to go to a key frame
...
const String &AVDecoder::Error() const
{
    return d->mError;
}
...
String AVDecoder::ContainerFormat() const
{
    if (!d->mAV.pFormat)
        return 0;
