...
struct QtProxyThread::Private : QThread
{
    ...
    std::atomic<Tiny::Thread *> mpTinyThread{ nullptr };
    WaitableEvent mRun, mDone;

    Private(QtProxyThread *p) : QThread(QApplication::instance()), mRun(true), mDone(false), mpSelf(p)
    {
        QThread::start();
    }
    ~Private()
    ...
    {
        mpSelf->p = nullptr;
        Thread* pTinyThread = mpTinyThread.load();
        if (pTinyThread)
            pTinyThread->Terminate();
        mRun.AbortWaiting();
        mDone.Set();
        QThread::wait();
    }
    bool Run(Runnable *pRunnable)
    {
        ...
        while (wait)
        {
            ...
            if (!mpTinyThread)
                return false;
            Runnable* value = nullptr;
            wait = !mpRunnable.compare_exchange_strong(value, pRunnable);
        }
        mDone.Reset();
        mRun.Set();
        if (!mDone.Wait())
    ...
    void ThreadFunc()
    {
        ...
        mDone.Set();
        WithThreadPriority(ThreadUtils::Priority::Maximum) while (mRun.Wait())
        {
        ...
        delete mpTinyThread.exchange(nullptr);
    }
};
