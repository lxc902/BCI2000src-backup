//////////////////////////////////////////////////////////////////////
// $Id: UnitTest.cpp 6484 2022-01-03 16:59:03Z mellinger $
...
static void DefaultHandler(const string &s)
{
    cerr << s << endl;
}

static string MakeDesc(const string &inName)
{
    string desc = inName, lcName = inName;
...
Test::Test(const string &inName) : mDesc(MakeDesc(inName))
{
    sTests.push_back(this);
}

ostream &Test::FailStream_(const char *inFile, int inLine)
{
    return mFailStream << mDesc << " failed in " << inFile << ", line " << inLine << ":\n";
}

bool Test::Run_(FailHandler_ inHandler)
{
    mFailStream.clear();
    mFailStream.str("");
    OnRun_();
...
int Test::Parse_(int argc, char **argv, bool exitIfFound)
{
    bool found = false;
...
Test::RunAll_::RunAll_(Test::FailHandler_ inHandler) : failures(0)
{
#if DISABLE_UNITTEST
