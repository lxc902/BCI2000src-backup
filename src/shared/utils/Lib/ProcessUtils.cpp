////////////////////////////////////////////////////////////////////////////////
// $Id: ProcessUtils.cpp 6571 2022-02-15 17:57:18Z mellinger $
...
Lockable<std::recursive_mutex>& WorkingDirAccess()
{
    static Lockable<std::recursive_mutex> instance;
    return instance;
}
struct WD_ : std::string
{
    WD_()
    {
        WithLock(WorkingDirAccess()) GetProcessWDInternal(*this);
    }
};
...
std::string ProcessUtils::ProcessWD()
{
    std::string wd;
    bool ok = false;
    WithLock(WorkingDirAccess()) ok = GetProcessWDInternal(wd);
    if (!ok)
...
{
    bool ok = false;
    WithLock(WorkingDirAccess()) ok = SetProcessWDInternal(wd);
    if (!ok)
...
GlobalID::GlobalID(const std::string &inName, int inTimeout) : mHandle(NULL)
{
...
void GlobalID::Destroy()
{
    ...
    std::string *pName = reinterpret_cast<std::string *>(mHandle);
    ::sem_unlink(pName->c_str());
    delete pName;

#else // USE_POSIX_SEMAPHORES

    key_t semkey = ftok(reinterpret_cast<std::string *>(mHandle)->c_str(), 'b');
    if (semkey != -1)
