////////////////////////////////////////////////////////////////////////////////
// $Id: WildcardMatch.cpp 6562 2022-02-12 16:32:17Z mellinger $
...
void RunTests()
{
    ...
    for (size_t i = 0; i < sizeof(sPositiveCases) / sizeof(*sPositiveCases); ++i)
    {
        ...
        if (!ExtWildcardMatch(pCase->pattern, pCase->match))
            throw std_exception << "ExtWildcardMatch test case failed: \"" << pCase->pattern << "\" does not match \""
                                << pCase->match << "\"";
    }
    for (size_t i = 0; i < sizeof(sNegativeCases) / sizeof(*sNegativeCases); ++i)
    {
        ...
        if (ExtWildcardMatch(pCase->pattern, pCase->match))
            throw std_exception << "WildcardMatch test case failed: \"" << pCase->pattern << "\" should not match \""
                                << pCase->match << "\"";
    }
