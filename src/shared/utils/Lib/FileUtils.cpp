//////////////////////////////////////////////////////////////////////
// $Id: FileUtils.cpp 6705 2022-04-30 12:18:20Z mellinger $
...
{
    return FileUtils::ParentDirectory(FileUtils::ExecutablePath());
}
...
std::string FileUtils::AbsolutePath(const std::string &inPath)
{
    if (inPath.empty())
        return inPath;
...
std::string FileUtils::CanonicalPath(const std::string &inPath)
{
    ...
    std::wstring wpath = StringUtils::ToWide("\\\\?\\" + path.ToString("\\"));
...
bool FileUtils::IsWritableDirectory(const std::string &inPath)
{
    if (!IsDirectory(inPath))
        return false;
    std::string testfile = WorkingDirectory() + "." + ExtractFile(ExecutablePath()) + ".readtest";
