////////////////////////////////////////////////////////////////////////////////
// $Id: sockstream.cpp 6571 2022-02-15 17:57:18Z mellinger $
...
////////////////////////////////////////////////////////////////////////////////
socket::socket()
    : fd_object(-1), m_listening(false), m_max_msg_size(0), m_address_type(0), m_pristine(false),
...
bool socket::set_address(const std::string &address)
{
    std::string hostname;
...
bool socket::set_address(const std::string &inIP, unsigned short inPort)
{
    std::string ip = inIP;
    if (ip == "localhost")
...
int64_t fd_object::seek_to(int64_t off)
{
...
int fd_object::on_seek_to(int64_t &off)
{
...
int streambuf::sync_read()
{
    if (gptr() < egptr())
    {
        std::streampos newpos = on_seek_to(m_readpos);
        if (newpos == m_readpos)
            setg(0, 0, 0);
    }
    return 0;
...
std::streampos streambuf::seek_to(std::streampos pos)
{
    if (sync_write() < 0)
        return -1;
    std::streampos newpos = on_seek_to(pos);
    if (newpos < 0)
        return -1;
    m_writepos = newpos;
    m_readpos = newpos;
...
std::streampos fd_streambuf::on_seek_to(std::streampos pos)
{
