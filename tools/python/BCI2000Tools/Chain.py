...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Chain.py 6662 2022-04-07 00:32:03Z jhill $
...
def bci2000chain(datfile, chain='SpectralSignalProcessing', parms=(), dims='auto', start=None, duration=None, verbose=False, keep=False, binpath=None, callback=None, **kwargs):
	"""
	...
	def process(cmd, callback, logfile): # block-by-block processing with a callback
		...
		try:
			while not stderr.configPassed and not stderr.configFailed: time.sleep( 0.001 )
			while not stderr.configFailed:
				line = pipe.readline()
				if not line: break
				#sys.stderr.write( bytes2str( line ) ); sys.stderr.flush()
				line = line.strip()
		...
		if status == 0: # only call the callback again if everything's OK
			callback( d ) # d['Changed'] will be empty, to signal to the callback that it should wrap things up, if necessary
...
class CallbackObject( object ):
	def __init__( self, stream=sys.stderr, announcementPeriodInSeconds=2, debug=False, **counters ):
