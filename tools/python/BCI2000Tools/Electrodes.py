...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Electrodes.py 6769 2022-05-28 02:00:58Z jhill $
...
Example::
	...
	from BCI2000Tools.Electrodes import ChannelSet, SCHEMES
	gnd = 'AFz'
	ref = 'TP9+TP10'
	spec = '\n'.join( '%-6s %s %s' % ( name, ref, gnd ) for name in SCHEMES['extended1020'].split() if name.lower() != gnd.lower() and name.lower() not in ref.lower().split('+') )
	# spec is a plain-text table showing where each electrode is positioned, referenced and grounded
	
	c = ChannelSet(spec)

	import numpy, matplotlib.pyplot as plt
	plt.subplot(1,2,1); c.plot()
	plt.subplot(1,2,2); c.plot( act=numpy.arange(c.size) )

Alternative::
	from BCI2000Tools.Electrodes import ChannelSet
	
	channels = 'P3 C3 F3 Fz F4 C4 P4 Cz A1 Fp1 Fp2 T7 P7 O1 O2 F7 F8 A2 P8 T8 TRG-0@0'
	c = ChannelSet( channels.split(), ref='Pz', gnd='AFz' )
...
from . import ElectrodePositions; from .ElectrodePositions import schemes as SCHEMES  # TODO: ideally electrode positions should come from some *extensible* set of resource files on the path, rather than a fixed Python module
...
class ChannelSet(numpy.matrix):
	...
	def stplot(self, img, fs=None, drawnow=True, **kwargs):
		...
		def samples2msec( samples, fs ): return 1000.0 * t / float( fs )
		if fs is None: x = None; xlabel = 'time-sample index'
		else: x = t = samples2msec( t, fs ); xlabel = 'time (msec)'
		x = None
		plt.subplot(121)
		h = Plotting.imagesc(img, x=x, aspect='auto', drawnow=False, picker=5, **kwargs)
		ax = plt.gca()
		if x is None and fs is not None:
			xl = ax.get_xlim()
			xt = numpy.array([x for x in ax.get_xticks() if min(xl)<=x<=max(xl)])
			xtl = ['%g' % x for x in samples2msec(xt, fs)]
		...
		h.timebase = t
		h.spaceax = plt.subplot(222)
		h.timeax = plt.subplot(224)
		def onpick(evt):
			...
			plt.axes(h.timeax)
			Plotting.plot(h.timebase, h.img[row], drawnow=False)
			plt.gca().set(ylim=h.get_clim(), xlim=(h.timebase[0],h.timebase[-1]))
			plt.draw()
	...
	def set_plotopts(self, d=None, **kwargs):
		if d is not None: self.__plotopts = d
		if self.__plotopts is None: self.__plotopts = {}
	...
	def RerefMatrix(self, newReferenceLabels, exclude='auto', keep='all', filters_as='columns', coords=None):
		...
		if isinstance( newReferenceLabels, str ):
			newReferenceLabels = newReferenceLabels.replace( ',', ' ' ).replace( '+', ' ' ).split()
		newReferenceIndices = []
		for label in newReferenceLabels:
			try: newReferenceIndex = labels.index( label.lower() )
			except ValueError: raise ChannelNotFound( "cannot use %r as a reference as it is not in the montage" % label )
...
def trodeplot(channels=(), act='connected',
	...
	if len(act.shape) > 1 and act.shape[1] > 1:
		...
		if i+1 < ax.size:
			ax.flat[i+1].cla()
			ax.flat[i+1].add_artist(plt.Text(0,0, text='Fin', fontname='fantasy',clip_on=False, size=20,visible=True))
	...
	if surf or contour:
		...
		chx,chy,chz = zip(*[p+(act[i],) for i,p in enumerate(positions) if p is not None])	
	...
	if nose:
		...
		h['nose'] = plt.Polygon(nxy, closed=False, edgecolor=color, facecolor='None', clip_on=False, zorder=0); ax.add_patch(h['nose'])
		#h['nose'] = Plotting.curve(nxy, edgecolor=color, facecolor='None', clip_on=False, zorder=0, hold=True)
	...
	if ears:
		...
		exy = numpy.c_[x,y]
		right = plt.Polygon(exy,            closed=False, edgecolor=color, facecolor='None', clip_on=False, zorder=0); ax.add_patch(right)
		left  = plt.Polygon(exy * [[-1,1]], closed=False, edgecolor=color, facecolor='None', clip_on=False, zorder=0); ax.add_patch(left)
		#right = Plotting.curve(exy,            edgecolor=color, facecolor='None', clip_on=False, zorder=0, hold=True)
		#left  = Plotting.curve(exy * [[-1,1]], edgecolor=color, facecolor='None', clip_on=False, zorder=0, hold=True)
		h['ears'] = [left,right]
...
def get_position(label, type='schematic2D', default=(numpy.nan,numpy.nan), coords=None):
	if coords is None: return getattr(ElectrodePositions, type).get(label.lower(), default)
