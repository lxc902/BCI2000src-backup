...
# ///////////////////////////////////////////////////////////////////////////
# $Id: FileReader.py 6662 2022-04-07 00:32:03Z jhill $
...
def ParseParam(param):
	...
	if datatype in ('int', 'float'):
		...
		if isinstance(unscaled, (bytes, unicode ,type(None))): Warn('failed to interpret "%s" as type %s in parameter "%s"' % (val,datatypestr,name))
		rec.update({
			'valstr'     : val,
			'val'        : unscaled,
			'units'      : units,
		...
		if valtype==str:
			...
			for u,v,s in zip(unscaled,val,eachstr):
				if isinstance(u, (bytes,unicode,type(None))): Warn( 'failed to interpret "%s" as type %s in parameter "%s"' % (s,valtypestr,name) )
		rec.update({
			'valstr'     : valstr,
			'valtype'    : valtype,
			'len'        : numel,
