...
# ///////////////////////////////////////////////////////////////////////////
# $Id: Numerics.py 6642 2022-03-29 03:17:13Z jhill $
...
def pdiff( x, order=1, axis=-1, pBefore=0.5, padVal=0 ):
	...
	which defaults to 0.5. For odd values of `order`, the number of
	values to be padded at the beginning is rounded, with 0.5 always
	being rounded up---so, by default, with `order=1`, the result
	will be padded with a single zero at the beginning and nothing
	at the end.
...
def edges( x, axis=-1, direction=+1, exceed=0.0, return_logical=False ):
	...
	in, e.g.  `edges(signal > threshold)`, but it is also possible to
	customize the `exceed` parameter, thereby defining an edge as any
	jump in the specified `direction` whose size is greater than
	`exceed`.
	
	With `return_logical=True`, instead of an array of indices, a
	logical array the same size as `x` is returned, wherein the edges
	are marked True.
	...
	edges = ( direction * pdiff( x, axis=axis ) > exceed )
