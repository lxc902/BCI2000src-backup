///////////////////////////////////////////////////////////////////////////////////
// $Id: BootstrapCustomProjects.cpp 6633 2022-03-18 22:12:18Z mellinger $
...
struct SourcePath
{
    SourcePath(const std::string &rel) : Rel(FileUtils::NormalizedPath(rel))
    {
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (std::regex_match(name, std::regex("new[[:alnum:]]*filterdll.*", std::regex::icase)))
            result = SetupFilterDll(argc, argv, interactive);
        else if (std::regex_match(name, std::regex("new[[:alnum:]]*filtertool.*", std::regex::icase)))
